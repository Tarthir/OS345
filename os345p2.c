...
extern Semaphore* tics10sec;				//sem for 10 sec timer
...
int P2_project2(int argc, char* argv[])
{
	...
	createTask("timingTask", timingTask,HIGH_PRIORITY,1,timeingArgv);
	createTask("timingTask2", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask3", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask4", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask5", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask6", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask7", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask8", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask9", timingTask, HIGH_PRIORITY, 1, timeingArgv);
	createTask("timingTask10", timingTask, HIGH_PRIORITY, 1, timeingArgv);
...
int signalTask(int argc, char* argv[])
{
	...
	char *ret;
	Semaphore** mySem;
	if((ret = strstr(argv[1], "s1Sem")) != NULL) {
		mySem = &s1Sem;
	}
	else {
		mySem = &s2Sem;
	}
	//Semaphore** mySem = (!strstr(argv[1], "s1Sem")) ? &s1Sem : &s2Sem;
