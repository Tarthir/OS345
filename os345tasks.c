					...
					char* argv[])				// task argument pointers
{
	...
	for (tid = 0; tid < MAX_TASKS; tid++)
	{
		...
		if (taskSems[tid]) deleteSemaphore(&taskSems[tid]);
			...
			tcb[tid].RPT = LC3_RPT + ((tid) ? ((tid - 1) << 6) : 0); //root page table
			tcb[tid].timeRemaining = 0;
			...
			{
				//printf("\n%s",argv[i]);
				tcb[tid].argv[i] = (char*)malloc(sizeof(char)*strlen(argv[i])+1);// +1 for null terminator
				
				strncpy(tcb[tid].argv[i], argv[i],strlen(argv[i]));	//cpy arg ///TODO issue with arguments being freed
				tcb[tid].argv[i][strlen(argv[i])] = 0;// add nul terminator
				//printf("\n%s", tcb[tid].argv[i]);
			}
...
int killTask(int taskId)
{
	if (taskId != 0)			// don't terminate shell
	{
		if (taskId < 0)			// kill all tasks
		{
			...
			{
				if (tcb[tid].name) {
					tcb[tid].state = S_EXIT;		//set task to exit state
					exitTask(tid); 
				}
			}
		}
...
static void exitTask(int taskId)
{
	...
	//}
	//}
	//if (taskId == 8 || taskId == 9 || taskId == 10 || taskId == 11) {///DOING this just only makes 1 exit
