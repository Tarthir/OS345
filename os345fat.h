...
//really only dealing with read only and archive
...
typedef struct
{
	//these 3 come from getDirEntry:
	uint8	name[8];	      	// file name
	uint8	extension[3];		// extension
	uint8	attributes;		   	// file attributes code

	uint16	directoryCluster;	// directory cluster, set to cDir
	uint16	startCluster;		// first cluster of the file, come from directory dirEntry.startCluster
	uint16	currentCluster;		// current cluster in buffer, 0
	uint32	fileSize;	   		// file size in bytes //if mode == 1 ? 0 : dirEntry.fileSize. When writing set to zero
	int		pid;				// process who opened file, curTask
	char	mode;				// access mode (read, read-only, write, append)
	char	flags;				// flags
								//   x80 = file altered (only can close now, dirty bit)
								//   x40 = buffer altered
								//   x20 = locked
								//   x10 =
								//   x08 = write protected
								...
								//   x02 =
								//   x01 =
	uint32	fileIndex;			// next character position (from beg of file). if mode != 2 ? 0 : dirEntry.fileSize. append mode points pointer at end of file
