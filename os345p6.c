...
int P6_dfat(int argc, char* argv[])		// list FAT table
{
	...
	switch (argc)
	{
		...
		case 2:						//	2	fat <1 to 2>		output fat <#> table
		{
			...
			if ((index == 1) || (index ==2))
			{
				sprintf_s(buf, "Disk Image: FAT %d", index);
...
int fmsTests(int test, bool debug)
{
	...
	switch (test)
	{
		...
		case 3:
		{
			...
			printf("\nRunning Test 3...");
			sprintf_s(buf2, "file%d.txt", test2);
			for (i=0; i<numWords; i++)
			{
				sprintf_s(buf, "file%d.txt", i);
				// open word file
		...
		case 4:
		{
			...
			printf("\nRunning Test 4...");
			sprintf_s(buf2, "file%d.txt", test3);
...
int P6_read(int argc, char* argv[])		// read file
{
	...
	if (nBytes == 512) {
		...
		nBytes %= 512; //MIGHT NEED TO DO JUST THIS? instead of if/else
	}
...
// ***************************************************************************************
// ***************************************************************************************
//dirNum is iterator to file
//dir taks current directory tcb[curTask]->cDir
