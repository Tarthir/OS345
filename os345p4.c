...
extern int outerClock;
extern int innerClock;
...
{
	int nextpage = nextPage;//accessPage(0, 0, PAGE_GET_SIZE);
	int pagereads = pageReads;//accessPage(0, 0, PAGE_GET_READS);
	int pagewrites = pageWrites;// accessPage(0, 0, PAGE_GET_WRITES);
...
void displayPT(int pta, int badr, int inc)
{
	...
	for (i=0; i<32; i++)
	{
      sprintf_s(buf, "(x%04x-x%04x) ", badr+ i*inc, badr + ((i+1)*inc)-1);
		outPTE("", (pta + i*2));
	}
...
void loadLC3File(char* string) //ERROR looking for file
{
	char* myArgv[2];
	char buff[32];
